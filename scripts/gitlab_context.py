...
DEFAULT_REQUIREMENT_PATTERNS = [
    ...
    r"\bREQ-\d+\b",
    r"#\d+\b",
    r"(?:需求|任务|缺陷|BUG|Bug|bug)[:：\s#-]*([A-Za-z0-9_-]{3,})",
]
...
def extract_requirement_ids(mr, extra_patterns=None):
    patterns = list(DEFAULT_REQUIREMENT_PATTERNS)
    if extra_patterns:
...
def summarize_approval_state(approval_state, approvals):
    result = {
        "status": "unknown",
        "approved": None,
        "approved_by": [],
    ...
    if isinstance(approval_state, dict):
        rules = approval_state.get("rules") or []
        normalized_rules = []
        required_rules = []
        approved_by = []
        for rule in rules:
            if not isinstance(rule, dict):
                continue
            approvals_required = int(rule.get("approvals_required") or 0)
            approved = bool(rule.get("approved"))
            users = rule.get("approved_by") or []
            for item in users:
                user = item.get("user") if isinstance(item, dict) else None
                if isinstance(user, dict):
                    approved_by.append(normalize_author(user))
            normalized = {
                "name": rule.get("name", ""),
                "approved": approved,
                "approvals_required": approvals_required,
                "approvals_left": rule.get("approvals_left"),
            }
            normalized_rules.append(normalized)
            if approvals_required > 0:
                required_rules.append(normalized)

        result["rules"] = normalized_rules
        result["approved_by"] = dedupe_people(approved_by)
        if required_rules:
            result["approved"] = all(rule["approved"] for rule in required_rules)
            result["status"] = "approved" if result["approved"] else "not_approved"
        elif normalized_rules:
            result["approved"] = True
            result["status"] = "no_required_rules"

    if result["status"] == "unknown" and isinstance(approvals, dict):
        approved = approvals.get("approved")
        result["approved"] = bool(approved) if approved is not None else None
        result["status"] = "approved" if approved else "not_approved"
        approved_by = []
...
def normalize_mr(mr, requirement_patterns=None):
    ...
    return {
        ...
        "requirement_ids": extract_requirement_ids(mr, requirement_patterns),
    }


def merge_context(context, gitlab_context):
    selected = gitlab_context.get("selected_merge_request") or {}
    approval = gitlab_context.get("approval") or {}
    ...
    merged["gitlab_mr_source_branch"] = selected.get("source_branch", "")
    merged["gitlab_mr_target_branch"] = selected.get("target_branch", "")
    merged["gitlab_mr_labels"] = selected.get("labels", [])
    merged["gitlab_mr_web_url"] = selected.get("web_url", "")
    merged["gitlab_approval_status"] = approval.get("status", "")
    merged["gitlab_approved_by"] = approval.get("approved_by", [])
    merged["requirement_ids"] = selected.get("requirement_ids", [])
    return merged


def collect_gitlab_context(args, context):
    project_url = first_non_empty(args.project_url, context.get("gitlab_project_url"), os.getenv("GITLAB_PROJECT_URL"))
    api_url = first_non_empty(args.api_url, os.getenv("GITLAB_API_URL"), infer_api_url(project_url))
    project_raw = first_non_empty(args.project_id, context.get("gitlab_project_id"), os.getenv("GITLAB_PROJECT_ID"))
    ...
    commit = first_non_empty(args.commit, context.get("deploy_commit"), os.getenv("DEPLOY_COMMIT"))
    preferred_iid = first_non_empty(args.mr_iid, context.get("gitlab_mr_iid"), os.getenv("GITLAB_MR_IID"))
    target_branch = first_non_empty(context.get("deploy_branch"), os.getenv("DEPLOY_BRANCH"))
    ...
    base = {
        ...
        "approval": {"status": "unknown", "approved": None, "approved_by": [], "rules": []},
    }
    ...
    if not commit and not preferred_iid:
        missing.append("DEPLOY_COMMIT or GITLAB_MR_IID")
    if not token:
    ...
    try:
        ...
        if selected and selected.get("iid"):
            approval_state = client.get(
                f"/projects/{project}/merge_requests/{selected['iid']}/approval_state",
                allow_missing=True,
            )
            approvals = client.get(
                f"/projects/{project}/merge_requests/{selected['iid']}/approvals",
                allow_missing=True,
            )

        requirement_patterns = args.requirement_pattern or []
        base["status"] = "ok"
        base["merge_requests"] = [normalize_mr(item, requirement_patterns) for item in merge_requests]
        base["selected_merge_request"] = normalize_mr(selected or {}, requirement_patterns)
        base["approval"] = summarize_approval_state(approval_state, approvals)
...
def main():
    parser = argparse.ArgumentParser(description="Collect GitLab MR context for Jenkins deploy gate.")
    parser.add_argument("--context", default=os.getenv("GATE_DEPLOY_CONTEXT", ""))
    ...
    parser.add_argument("--requirement-pattern", action="append", default=[])
    ...
    if args.strict and gitlab_context.get("status") not in ("ok",):
        print("; ".join(gitlab_context.get("errors") or ["GitLab context collection failed"]), file=sys.stderr)
        raise SystemExit(1)
